...
def create_anki_decks(file_path):
    ...
    for index, line in enumerate(lines):
        ...
        if len(parts) < 2:
            raise ValueError(
                f"Error on line {index + 1}: does not have a tab to split into 2 parts."
            )
...
if __name__ == "__main__":
    # run code in powershell in same directory as script in the most simple implementation as: python "Deck generator.py"
    # Default file paths
    input_file_path = "input/raw deck names.txt"
    output_file_path = "output/anki decks and subdeck names.txt"
        ...
        with open(output_file_path, "w") as output_file:
            ...
            for i, deck in enumerate(decks):
                if i < len(decks) - 1:
                    ...
                    output_file.write(deck)
        print(
            f"Decks and subdecks have been successfully written to '{output_file_path}'"
        )
